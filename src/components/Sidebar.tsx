...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  return (
    <div className={cn("w-64 bg-sidebar text-sidebar-foreground min-h-screen p-4 flex flex-col shadow-lg", className)}>
          ...
          {parishes.map((parish) => (
            <Link key={parish.id} to={`/paroisse/${parish.name}`}>
              ...
              >
                ...
                {parish.name}
                <span className="ml-auto bg-sidebar-primary/30 text-sidebar-foreground/90 px-1.5 py-0.5 rounded text-xs truncate max-w-[40px]">
                  {parish.memberCount}
                </span>
              </Button>
            </Link>
